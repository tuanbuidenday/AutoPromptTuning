...
def test_tat_ca_dung_duoc_100(samples):
...
def test_mot_nua_dung_duoc_50(samples):
    ev = AccuracyEvaluator()
...
def test_khong_phan_biet_hoa_thuong_mac_dinh(samples):
    ev = AccuracyEvaluator()
    r = ev.evaluate("p", _preds("yes", "NO", "YES", "no"), samples)
    assert r.score == 100.0


def test_case_sensitive_bat_loi_khac_hoa_thuong(samples):
    ev = AccuracyEvaluator(case_sensitive=True)
    r = ev.evaluate("p", _preds("yes", "No", "Yes", "No"), samples)
    assert r.score == 75.0


def test_bo_qua_ca_bi_loi_llm(samples):
...
def test_bo_qua_ca_khong_co_nhan():
    ev = AccuracyEvaluator()
...
def test_thieu_du_doan_tinh_la_sai(samples):
    ev = AccuracyEvaluator()
