...
def test_save_tang_version_tu_dong(store):
...
def test_record_score_va_history(store):
    v0 = store.save("A")
    v1 = store.save("B")
...
def test_best_tra_ve_diem_cao_nhat(store):
...
def test_best_la_none_khi_chua_cham_diem(store):
    store.save("A")
    assert store.best() is None


def test_metadata_duoc_giu(store):
    store.save("A", {"source": "initial"})
    assert store.history()[0].metadata["source"] == "initial"


def test_sqlite_ben_vung_sau_khi_mo_lai(tmp_path):
...
def test_sqlite_tach_biet_theo_run_name(tmp_path):
